...
# Compute column statistics on all 24 TPC-DS tables via ADD STATICS.
#
# Run this after generate-data.sh and before run-benchmark.sh.
#
# Usage:
#   ./create-statics.sh [SPARK_MASTER]
...
echo "=== TPC-DS Statistics Creation ==="
...
# Step 3: Create statistics via spark-submit
...
${SPARK_SUBMIT} \
  --class org.lance.spark.benchmark.TpcdsStatisticsBuilder \
  --master "${SPARK_MASTER}" \
...
echo ""
echo "=== Statistics creation complete ==="
