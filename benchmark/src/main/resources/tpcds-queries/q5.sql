WITH ssr AS (
  ...
  WHERE date_sk = d_date_sk AND d_date BETWEEN CAST('2000-08-23' AS DATE) AND DATE_ADD(CAST('2000-08-23' AS DATE), 14)
...
FROM (
  SELECT 'store channel' AS channel, CONCAT('store', s_store_id) AS id, sales, returns_val, (profit - profit_loss) AS profit FROM ssr
  UNION ALL
  SELECT 'catalog channel' AS channel, CONCAT('catalog_page', cp_catalog_page_id) AS id, sales, returns_val, (profit - profit_loss) AS profit FROM csr
  UNION ALL
  SELECT 'web channel' AS channel, CONCAT('web_site', web_site_id) AS id, sales, returns_val, (profit - profit_loss) AS profit FROM wsr
) x
