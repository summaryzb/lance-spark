...
public class LanceFragmentScanner implements AutoCloseable {
  ...
  /**
   * Holds the miss-column subset published by the executor disk-cache loader so that the lazy
   * {@link #scannerSupplier} factory can read it on first {@link LazyResource#get()} and project
   * only those columns to Lance. {@code getAndSet(null)} inside the factory clears the slot
   * immediately so a stale value cannot leak into a future scanner build. Stays {@code null} on
   * full-cache-hit (lambda never fires) and on the non-cache path (L{@code 221}); both paths fall
   * back to the full schema. Intentionally not {@code private} so that
   * {@link LanceFragmentScannerTest} can verify the lazy-injection contract via reflection.
   */
  final AtomicReference<List<String>> pendingMissCols;
  ...
  private LanceFragmentScanner(
      ...
      AtomicReference<List<String>> pendingMissCols) {
    ...
    this.pendingMissCols = pendingMissCols;
  }
  ...
  private static LanceFragmentScanner buildScanner(
    ...
    boolean withFragmentId =
        inputPartition.getSchema().getFieldIndex(LanceConstant.FRAGMENT_ID).nonEmpty();
    AtomicReference<List<String>> pendingMissCols = new AtomicReference<>();
    LazyResource<LanceScanner> scannerSupplier =
        new LazyResource<>(
            () ->
                buildLanceScanner(
                    fragmentId, inputPartition, datasetSupplier, pendingMissCols.getAndSet(null)));
    return new LanceFragmentScanner(
        datasetSupplier,
        scannerSupplier,
        fragmentId,
  ...
  private static LanceScanner buildLanceScanner(
    ...
    List<String> projectedColumns = extractColNames(inputPartition.getSchema(), overrideCols);
      ...
      try {
        return LanceExecutorCache.getInstance()
            .getOrLoadColumns(
                ...
                allocator,
                missCols -> {
                  // Publish the cache-computed miss subset so the lazy scanner factory projects
                  // only those columns to Lance instead of the full schema. Cleared by
                  // pendingMissCols.getAndSet(null) inside the factory closure.
                  pendingMissCols.set(missCols);
                  return scannerSupplier.get().scanBatches();
                });
      } catch (IOException e) {
  ...
  /**
   * Resolves the projection column list for {@link #buildLanceScanner}. When {@code overrideCols}
   * is non-null (the cache-loader path with a computed miss subset), returns those columns
   * verbatim; otherwise falls back to the full schema-derived projection.
   *
   * <p>The override is already a column-name subset of {@link #getColumnNames} that preserves the
   * original relative ordering (the cache iterates {@code requestedColumns} linearly when
   * computing miss columns), so no further filtering or reordering is needed here. We make a
   * defensive copy because the cache may hand us a shared / unmodifiable list and the scan-options
   * builder otherwise holds an alias.
   */
  private static List<String> extractColNames(StructType schema, List<String> overrideCols) {
    if (overrideCols == null) {
      return getColumnNames(schema);
    }
    return new ArrayList<>(overrideCols);
  }
